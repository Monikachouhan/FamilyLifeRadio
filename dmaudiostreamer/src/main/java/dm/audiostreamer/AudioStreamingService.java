    ...
    private void createNotification(MediaMetaData mSongDetail) {
        try {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                NotificationChannel channel = new NotificationChannel("family", "Family Radio", android.app.NotificationManager.IMPORTANCE_LOW);
                channel.setLockscreenVisibility(Notification.VISIBILITY_PUBLIC);
                channel.setShowBadge(false);
                ((android.app.NotificationManager) getSystemService(NOTIFICATION_SERVICE)).createNotificationChannel(channel);
            }
            if (pendingIntent != null) {
                notification = new NotificationCompat.Builder(getApplicationContext(), "family").setSmallIcon(R.drawable.player)
